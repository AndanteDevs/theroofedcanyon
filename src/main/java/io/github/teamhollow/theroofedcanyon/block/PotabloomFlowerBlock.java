...
public class PotabloomFlowerBlock extends Block {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand,BlockHitResult hit) {
        ...
        if ((int)state.get(VILE_LEVEL) > 0 && itemStack.getItem() == Items.GLASS_BOTTLE) {
    ...
    public void takeVile(World world, BlockState state, BlockPos pos) {
        world.setBlockState(pos, (BlockState)state.with(VILE_LEVEL, state.get(VILE_LEVEL) - 1), 3);
    }

    @Environment(EnvType.CLIENT)
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if ((int)state.get(VILE_LEVEL) >= 4) {
            for (int i = 0; i < random.nextInt(1) + 1; ++i) {
                this.spawnVileParticles(world, pos, state);
            }
        }
    ...
    @Environment(EnvType.CLIENT)
    private void spawnVileParticles(World world, BlockPos pos, BlockState state) {
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        int vileLevel = (int)state.get(VILE_LEVEL);
